...
def train_model(
    ...
    signals_file = output_path / f"signals_{symbol.lower()}.csv"
    signals_df.to_csv(signals_file, index=False)

    model_file = output_path / f"model_{symbol.lower()}.joblib"
...
def run_prediction(
    ...
    client = create_client()
    data = client.fetch_historical(symbol, interval, fetch_bars)
...
def run_backtest(
    ...
    price_data = client.fetch_historical(symbol, interval, bars_needed)
    ...
    return {
        ...
        "winning_trades": int(trades["pnl"].sum() > 0) if len(trades) > 0 else 0,
