...
class AdaptiveRegimeStrategy(MLStrategy):
    ...
    def __init__(
        self, symbols: list[str], aggregator, regime_detector: RegimeDetector = None
    ):
        super().__init__(symbols, aggregator)
        self.regime_detector = regime_detector or RegimeDetector()

    def generate_signals(
        self, data_handler: DataHandler, portfolio: PortfolioManager
    ) -> list[Order]:
    ...
    def _signal_to_orders(
        self, signal: Signal, portfolio: PortfolioManager, data_handler: DataHandler
    ) -> list[Order]:
        ...
        for order in orders:
            ...
            if regime.name == "volatile":
                ...
                order.stop_loss = (
                    price * 0.97 if order.side == Side.BUY else price * 1.03
                )
                order.size *= 0.5
            elif regime.name == "ranging":
                # Reduce target profit
                order.take_profit = (
                    price * 1.02 if order.side == Side.BUY else price * 0.98
                )
                order.size *= 0.7
